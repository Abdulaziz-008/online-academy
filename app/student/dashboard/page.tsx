...
import { mockCourses, studentData } from '@/lib/data'

export default function StudentDashboard() {
  const { user } = useAuth()

  const data = studentData[user?.id || '2'] || studentData['2']
  const { purchasedIds, progresses, hoursLearned, testsPassed, streak, recentActivity } = data
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      >
        ...
        <div className="relative">
          <div className="text-sm text-sky-400 font-semibold mb-1">Xush kelibsiz! 👋</div>
          <h2 className="font-display font-bold text-2xl mb-2" style={{ color: 'var(--color-text)' }}>
            {user?.name}
          </h2>
          <p className="text-sm mb-4" style={{ color: 'var(--color-text-muted)' }}>
            Bugun ham o'rganishni davom ettiring. Siz {avgProgress}% yo'lni bosib o'tgansiz!
          </p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Kurslarim', value: enrolledCourses.length, icon: <BookOpen size={18} />, color: '#0ea5e9' },
          { label: "O'rganilgan soatlar", value: hoursLearned, icon: <Clock size={18} />, color: '#f97316' },
          { label: 'Testlar topshirildi', value: testsPassed, icon: <Award size={18} />, color: '#22c55e' },
          { label: 'Streak (kun)', value: `${streak}🔥`, icon: <TrendingUp size={18} />, color: '#a855f7' },
        ].map((s, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <Card
            ...
            action={
              <a href="/student/courses" className="text-xs text-sky-400 hover:text-sky-300 flex items-center gap-1">
            ...
            {enrolledCourses.length === 0 ? (
              <div className="py-12 text-center">
                ...
                <p className="text-sm" style={{ color: 'var(--color-text-muted)' }}>
                  Hozircha kurs sotib olinmagan
                </p>
                <a href="/student/courses" className="text-xs text-sky-400 hover:text-sky-300 mt-2 inline-block">
                  Kurslarni ko'rish →
                </a>
              </div>
            ) : (
        ...
        <div className="space-y-6">
          {/* Upcoming tests */}
          <Card title="Testlar" subtitle="Kutilayotgan testlar">
            <div className="divide-y" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
              {upcomingTests.map((test, i) => (
                <div key={i} className="p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium leading-snug truncate" style={{ color: 'var(--color-text)' }}>
                        {test.title}
                      </div>
                      <div className="text-xs mt-0.5" style={{ color: 'var(--color-text-muted)' }}>
                        {test.course} · {test.questions} savol
                      </div>
          ...
          {/* Recent activity */}
          <Card title="Faollik" subtitle="So'nggi harakatlar">
            <div className="divide-y" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
